...
export const ahkDocSearchToolDefinition = {
  ...
  outputSchema: {
    ...
    required: ['query', 'category', 'pagination', 'results'],
  },
};
...
export class AhkDocSearchTool {
  ...
  private ensureIndex(): void {
    ...
    if (full?.data) {
      (full.data.Functions || []).slice(0, 10000).forEach((f, i) => {
        docs.push({
          id: `fdfn:${i}:${f.Name}`,
          type: 'function',
          name: f.Name,
  ...
  async execute(args: unknown): Promise<McpToolResponse> {
    ...
    const requestContext = {
      ...
      category: parsed.data.category ?? 'auto',
    };
    ...
    try {
      ...
      try {
        const sets = await AhkDocSearchTool.index.searchAsync<true>(query, {
          enrich: true,
          limit: maxToFetch * 2,
          index: ['name', 'description', 'path'],
        });
        ...
        for (const set of sets || []) {
          for (const unit of set.result || []) {
            const doc: IndexedDoc | undefined = unit.doc;
            if (!doc) continue;
